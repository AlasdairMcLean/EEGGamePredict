...
dataDir = 'C:\Users\JacekSuper\Dropbox\ARL\data\'
for s=1:nSubj
    tmp=dir([dataDir subjStrs{s} '/play/mat/*ready_data_jason.mat']);
    if numel(tmp)>maxRacesPerSubject, tmp=tmp(1:maxRacesPerSubject); end
    structfilenames_PLAY=cat(1,structfilenames_PLAY,tmp);
    
    tmp=dir([dataDir subjStrs{s} '/bci/mat/*ready_data_jason.mat']);
    if numel(tmp)>maxRacesPerSubject, tmp=tmp(1:maxRacesPerSubject); end
    structfilenames_BCI=cat(1,structfilenames_BCI,tmp);
    
    tmp=dir([dataDir subjStrs{s} '/watch/mat/*ready_data_jason.mat']);
    if numel(tmp)>maxRacesPerSubject, tmp=tmp(1:maxRacesPerSubject); end
